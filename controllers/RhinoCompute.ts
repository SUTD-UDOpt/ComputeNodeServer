...
export const processInputData = async (
  ...
  param13.append([0], [formData.allInitialEdgesAreRoad === 1 ? true : false]);
  const param14 = new RhinoCompute.Grasshopper.DataTree("IPathOnly");
  param14.append([0], [formData.iPathOnly === 1 ? true : false]);
  ...
  param19.append([0], [formData.vAnchorStrength === 0 ? 0 : Math.pow(10, formData.vAnchorStrength)]);
  const param20 = new RhinoCompute.Grasshopper.DataTree("vOnCrvStrength");
  param20.append([0], [formData.vOnCrvStrength === 0 ? 0 : Math.pow(10, formData.vOnCrvStrength)]);
  const param21 = new RhinoCompute.Grasshopper.DataTree("pAnchorStrength");
  param21.append([0], [formData.pAnchorStrength === 0 ? 0 : Math.pow(10, formData.pAnchorStrength)]);
  const param22 = new RhinoCompute.Grasshopper.DataTree("roadSmoothStrength");
  param22.append([0], [formData.roadSmoothStrength === 0 ? 0 : Math.pow(10, formData.roadSmoothStrength)]);
  const param23 = new RhinoCompute.Grasshopper.DataTree("roadJunctionAngleStrength");
  param23.append([0], [formData.roadJunctionAngleStrength === 0 ? 0 : Math.pow(10, formData.roadJunctionAngleStrength)]);
  const param24 = new RhinoCompute.Grasshopper.DataTree("parcelSideSmoothStrength");
  param24.append([0], [formData.parcelSideSmoothStrength === 0 ? 0 : Math.pow(10, formData.parcelSideSmoothStrength)]);
  const param25 = new RhinoCompute.Grasshopper.DataTree("parcelSideAngleStrength");
  param25.append([0], [formData.parcelSideAngleStrength === 0 ? 0 : Math.pow(10, formData.parcelSideAngleStrength)]);
  const param26 = new RhinoCompute.Grasshopper.DataTree("parcelSideLengthStrength");
  param26.append([0], [formData.parcelSideLengthStrength === 0 ? 0 : Math.pow(10, formData.parcelSideLengthStrength)]);
  const param27 = new RhinoCompute.Grasshopper.DataTree("parcelAdjacencyStrength");
  param27.append([0], [formData.parcelAdjacencyStrength === 0 ? 0 : Math.pow(10, formData.parcelAdjacencyStrength)]);
